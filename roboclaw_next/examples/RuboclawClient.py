            ...
            messages=[
                AgentMessage(
                    ...
                    content=(
                        ...
                        "fingertip direction, call it first: fingertips are wrist-local -Z. "
                        "From its quaternion compute current v=R(q)[0,0,-1] and target t in "
                        "arm_origin (+X forward, +Y left, +Z up), then use the shortest "
                        "axis-angle r=unit(v cross t)*acos(clamp(v dot t)) as rotation_vector "
                        "with orientation_mode=relative and rotation_frame=arm_origin. "
                        "Use keep when no rotation is requested, tolerant when variation is "
                        "allowed, and free only when any orientation is acceptable. "
                        "On failure retry once with a smaller rotation or, unless the user "
                        "fixed position, small XYZ offsets; stop on START_STATE_IN_COLLISION "
                        "and never execute failed plans. For maximum-range requests compare "
                        "several candidates and execute only the best. "
                    ),
                ),
            ]
        )
